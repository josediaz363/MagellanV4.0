            ...
            rows.forEach(row => {
                ...
                if (cells.length >= 5) { // Ensure we have enough cells
                    ...
                    if (budgetedIndex >= 0 && earnedIndex >= 0) {
                        ...
                        budgetedHours = parseFloat(cells[3].textContent.trim()) || 0;
                        
                        // Earned hours is typically the 5th column (index 4)
                        earnedHours = parseFloat(cells[4].textContent.trim()) || 0;
                    }
        ...
        try {
            ...
            rows.forEach(row => {
                ...
                if (cells.length >= 4) { // Ensure we have enough cells
                    // Try to find progress column by header text
                    const headers = subJobsTable.querySelectorAll('thead th');
                    let progressIndex = -1;
                    ...
                    if (progressIndex >= 0) {
                        const progressText = cells[progressIndex].textContent.trim();
                        progress = parseFloat(progressText) || 0;
                    } else {
                        // Fallback: try common position
                        const progressText = cells[cells.length - 3].textContent.trim();
                        progress = parseFloat(progressText) || 0;
                    }
                    
                    // For simplicity, we'll use equal weights for all sub jobs
                    // In a real implementation, you might want to weight by budgeted hours
            ...
            if (totalWeight > 0) {
                ...
                return 0; // Return 0 if no valid progress values
            }
        } catch (error) {
