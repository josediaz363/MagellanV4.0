...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateDashboardProgress() {
        try {
            ...
            console.log("Quantity columns not found or invalid, trying to get progress directly from Progress column");
            
            const progressIndex = headers.findIndex(h => h.includes('progress'));
            if (progressIndex >= 0) {
                console.log(`Found progress column at index ${progressIndex}`);
                
                // First, try to calculate weighted average based on budgeted quantities
                let totalBudgetedQuantityForWeighting = 0;
                let weightedProgress = 0;
                
                // Try to find budgeted quantity column again
                const budgetedQuantityIndex = headers.findIndex(h => h.includes('budgeted quantity'));
                
                if (budgetedQuantityIndex >= 0) {
                    console.log(`Using budgeted quantity column at index ${budgetedQuantityIndex} for weighting`);
                    
                    rows.forEach((row, index) => {
                        const cells = row.querySelectorAll('td');
                        if (progressIndex < cells.length && budgetedQuantityIndex < cells.length) {
                            const progressText = cells[progressIndex].textContent.trim().replace('%', '');
                            const progress = parseFloat(progressText) || 0;
                            
                            const budgetedQuantityText = cells[budgetedQuantityIndex].textContent.trim().split(' ')[0]; // Remove units
                            const budgetedQuantity = parseFloat(budgetedQuantityText) || 0;
                            
                            if (!isNaN(progress) && budgetedQuantity > 0) {
                                weightedProgress += progress * budgetedQuantity;
                                totalBudgetedQuantityForWeighting += budgetedQuantity;
                                console.log(`Row ${index + 1} progress: ${progress}%, budgeted quantity: ${budgetedQuantity}`);
                            }
                        }
                    });
                    
                    if (totalBudgetedQuantityForWeighting > 0) {
                        const avgProgress = weightedProgress / totalBudgetedQuantityForWeighting;
                        console.log(`Calculated weighted average progress by quantity: ${avgProgress.toFixed(2)}%`);
                        return avgProgress;
                    }
                }
                
                // If we couldn't weight by quantities, try to calculate from the work items directly
                // We'll use a simple average of the progress values
                console.log("Calculating simple average of progress values");
                ...
                if (itemCount > 0) {
                    const avgProgress = totalProgress / itemCount;
                    console.log(`Calculated simple average progress: ${avgProgress.toFixed(2)}%`);
                    return avgProgress;
                }
            }
            
            // If we still couldn't calculate progress, try to calculate from budgeted hours and earned hours
            // This is a last resort fallback
            console.log("Could not calculate from progress column, trying to calculate from budgeted hours and earned hours as last resort");
