    ...
    function initializeSubJobChart() {
        ...
        try {
            ...
            try {
                ...
                // Store the progress value in localStorage for the project page to use
                try {
                    // Extract the sub job ID from the URL
        ...
        try {
            ...
            if (subJobsTable) {
                ...
                if (rows.length === 0) {
                    ...
                    // NEW: Try to get progress from the dashboard
                    return getDashboardProgress();
                }
            ...
            // NEW: Try to get progress from the dashboard as a last resort
            return getDashboardProgress();
        } catch (error) {
            console.error("Error calculating project progress:", error);
            return getDashboardProgress(); // Try dashboard progress on error
        }
    }
    
    // Helper function to get progress from dashboard
    function getDashboardProgress() {
        try {
            console.log("Trying to get progress from dashboard");
            
            // Make an AJAX request to the dashboard page
            const xhr = new XMLHttpRequest();
            xhr.open('GET', '/', false); // Synchronous request
            xhr.send();
            ...
            if (xhr.status === 200) {
                ...
                // Look for the progress in the table
                const table = tempDiv.querySelector('table');
                if (table) {
                    const rows = Array.from(table.querySelectorAll('tbody tr'));
                    if (rows.length > 0) {
                        // Try to find the progress column
                        ...
                        if (progressIndex >= 0) {
                            // Get progress from the first row
                            const cells = rows[0].querySelectorAll('td');
                            if (progressIndex < cells.length) {
                                const progressText = cells[progressIndex].textContent.trim().replace('%', '');
                                const progress = parseFloat(progressText) || 0;
                                console.log(`Found progress from dashboard: ${progress}%`);
                                return progress;
                            }
                        }
                        
                        // If we couldn't find the progress column, try to calculate from hours
                        let budgetedIndex = -1;
                        let earnedIndex = -1;
                        ...
                        if (budgetedIndex >= 0 && earnedIndex >= 0) {
                            const cells = rows[0].querySelectorAll('td');
                            if (budgetedIndex < cells.length && earnedIndex < cells.length) {
                                const budgetedHours = parseFloat(cells[budgetedIndex].textContent.trim()) || 0;
                                const earnedHours = parseFloat(cells[earnedIndex].textContent.trim()) || 0;
                                
                                if (budgetedHours > 0) {
                                    const progress = (earnedHours / budgetedHours) * 100;
                                    console.log(`Calculated progress from dashboard hours: ${progress.toFixed(2)}%`);
                                    return progress;
                                }
                            }
                        }
            ...
            console.log("Could not get progress from dashboard");
            return 0;
        } catch (error) {
            console.error("Error getting progress from dashboard:", error);
            return 0; // Return 0 on error
        }
    }
